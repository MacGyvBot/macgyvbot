...
def test_attach_base_position_uses_median_depth_around_hand_pixel():
    state = TaskRuntimeState(
        logger_provider=lambda: FakeLogger(),
        publish_robot_status=lambda *args, **kwargs: None,
        publish_status_payload=lambda payload: None,
    ...
    assert result["position"]["z"] == 3.5


def test_attach_base_position_respects_meter_depth_encoding():
    state = TaskRuntimeState(
        logger_provider=lambda: FakeLogger(),
        publish_robot_status=lambda *args, **kwargs: None,
        publish_status_payload=lambda payload: None,
    )
    state.depth_image = np.full((20, 20), 0.5, dtype=np.float32)
    state.depth_encoding = "32FC1"
    state.intrinsics = {
        "fx": 100.0,
        "fy": 100.0,
        "ppx": 10.0,
