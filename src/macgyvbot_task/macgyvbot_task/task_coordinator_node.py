...
class TaskCoordinatorNode(Node):
    ...
    def _create_subscriptions(self):
        ...
        self.manual_gripper_width_topic = (
            str(self.get_parameter("manual_gripper_width_topic").value).strip()
            or MANUAL_GRIPPER_WIDTH_TOPIC
        )
        self.create_subscription(
            Float32,
            self.manual_gripper_width_topic,
            self._manual_gripper_width_cb,
            10,
        )
    ...
    def _log_startup(self):
        ...
        self.get_logger().info(
            f"manual gripper width 토픽: {self.manual_gripper_width_topic}"
        )
        self.get_logger().info(f"YOLO model: {self.yolo_model}")
        self.get_logger().info(f"grasp point mode: {self.grasp_point_mode}")

    def _manual_gripper_width_cb(self, msg):
        requested_width_mm = float(getattr(msg, "data", 0.0))
        source = "operator_ui"

        allowed, reason = self._manual_gripper_is_allowed()
        if not allowed:
            self._reject_manual_gripper(
                reason,
