...
| `/sam_yaw` | `macgyvbot_interfaces/srv/SAMYaw` | `macgyvbot_perception` `sam_yaw_service_node` | `macgyvbot_task` `task_coordinator_node` | SAM-depth-mask yaw estimation using color image, depth image, bbox, and target label; optional grasp point plus intrinsics enables response grasp width in millimeters; response returns yaw, optional width, optional debug image path, and error status |
