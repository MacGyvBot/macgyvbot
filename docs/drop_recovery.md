...
drop이 감지되면 현재 MoveIt goal을 취소하고 `drop_req`로 일반 dispatch를 막습니다. 기존 queue와 중단된 `TaskStep`은 보존하며, recovery가 성공하면 중단됐던 step을 자동으로 다시 실행한 뒤 남은 queue를 계속 처리합니다.
...
- task queue suspend/cancel: `TaskCoordinatorNode`의 queue 보존과 MoveIt goal cancel 경로
...
-> set drop_req and keep queued work
-> suspend the active task step when it stops
-> set recovery_mode = True
-> clear locked SAM/depth mask state
-> move to inspection pose
-> open gripper after inspection pose arrival
-> search up to 4 seconds for the selected target tool
...
-> keep holding the recovered tool
-> close any drawer that was already open for cleanup
-> return home without drawer placement
-> set recovery_mode = False
-> clear drop_req and automatically re-run the suspended task step
-> continue the remaining queue
```

During `recovery_mode`, normal task control commands such as pause, resume,
retry, cancel, and exit are rejected by the task coordinator. A new drop event is
still accepted and queued so recovery can restart from the latest drop payload.
...
Pick/drop recovery와 return/drop recovery는 모두 `run_drop_recovery()`를
사용합니다.

공통 흐름은 동일하며, `task_type`에 따라 다음 값만 달라집니다.

- target 선택 순서
- return recovery에서 observe 후 실제 보이는 tool label로 target을 갱신하는지 여부
- grasp 성공 후 drop monitor를 다시 시작할 때 사용하는 action (`bring` 또는 `return`)

## Pick Recovery

Pick/delivery 중 공구를 떨어뜨린 경우 `run_drop_recovery(task_type="pick")`가 호출됩니다.
...
1. 기존 queue를 보존하고 현재 step을 suspend합니다.
...
성공 후에는 중단됐던 delivery step을 자동으로 다시 실행합니다.
...
- recovery grasp 성공 뒤에는 공구를 계속 잡은 상태로 home에 복귀하고, 중단됐던 step을 다시 실행합니다.
...
Return 중 사용자에게 받은 공구 또는 staging된 공구를 떨어뜨린 경우 `run_drop_recovery(task_type="return")`가 호출됩니다.
...
4. 기존 YOLO/depth target resolver로 선택된 target을 찾습니다.
5. `PickTargetPlanner`로 grasp 가능성을 확인합니다.
6. recovery grasp를 수행합니다.
7. 성공하면 `held_tool`, `gripper_holding`을 유지한 상태로 home에 복귀합니다.
8. 중단됐던 return step을 다시 실행합니다.

`"unknown"` target은 recovery detection에서 찾지 못하면 실패로 처리합니다. 최고 confidence detection을 임의 target으로 선택하지 않습니다.
...
- grasp 이후에는 공구를 계속 잡은 상태로 home에 복귀하고, 중단됐던 return step을 다시 실행합니다.
...
- Recovery 실패 후 operator가 retry, manual recovery, cancel 중 선택할 수 있는 UI 정책을 추가합니다.
