...
- `macgyvbot_command`: GUI, STT, TTS, command parser, `/tool_command`,
  `/robot_task_control` 발행
...
- `resume`은 pause 상태를 해제하고 queue 앞 step부터 작업을 이어갑니다.
- hand grasp, tool mask lock 같은 long wait loop는 실행 중인 trajectory goal이
  없으므로 loop 내부에서 stop/pause event를 직접 확인합니다.
- `TaskControlCoordinator`는 step 생성 또는 실행 중 예외가 발생하면 worker
  thread가 조용히 종료되지 않도록 실패 상태를 발행합니다.
...
task node와 command node의 camera topic wiring은
`macgyvbot_config.topics`의 `CAMERA_COLOR_TOPIC`, `CAMERA_DEPTH_TOPIC`,
`CAMERA_INFO_TOPIC` 상수를 사용합니다. launch/config에서 topic을 바꾸는 흐름을
깨지 않도록 camera topic을 node 내부에 직접 하드코딩하지 않습니다.
