...
handoff pose에서는 사용자 손이 locked ROI/mask 근처에 있는지와 `.pkl` ML
classifier의 `grasp` 판정을 함께 확인합니다. 기본 설정에서는 ML raw/stable
상태가 모두 `grasp`이고 confidence가 기준값 이상이어야 `/human_grasped_tool`의
