...
Linux 설치 스크립트는 보통 `ollama` systemd service를 함께 등록합니다. 설치 후
LLM parser가 응답하지 않으면 아래처럼 서비스 상태를 확인합니다.

```bash
systemctl status ollama
```

서비스가 없는 환경에서는 별도 터미널에서 `ollama serve`를 실행한 뒤
`command_input_node`를 실행합니다. command parser는 기본적으로
`http://localhost:11434/api/generate`에 요청을 보냅니다.
...
말풍선과 TTS로 안내합니다. `정지`, `재개`, `홈 복귀`, `종료`도 LLM/parser action으로
분류합니다.

GUI의 Robot Status 패널 하단에는 `복귀`와 `종료` 버튼이 있습니다. `복귀`는
`홈위치로 가` 입력과 같은 경로로 Home 복귀 명령을 전달하고, `종료`는 기존
`exit` 명령 흐름으로 해석해 `/robot_task_control`에 종료 요청을 전달합니다.
로봇 task/control 쪽은 현재 작업 중단과 Home 복귀를 처리하고, command GUI는
요청 전달 후 정상 종료 경로로 GUI와 command node를 종료합니다.

제어 action은 아래 이름을 사용합니다.

- `pause`: 사용자가 “멈춰”, “정지”, “중단”, “스탑”처럼 말했을 때 발행되는 일시정지/정지 요청
- `resume`: 사용자가 “재개”, “다시 시작”, “계속해”처럼 말했을 때 해석되는 재개 요청
- `home`: 사용자가 “홈위치로 가”, “복귀해”, “원위치로 가”처럼 말했을 때 발행되는 Home 복귀 요청
...
`/robot_task_control`의 `exit` 요청으로 발행합니다. 로봇은 현재 작업을 중단한 뒤
Home joint pose로 복귀하고 그리퍼를 엽니다. command GUI는 `exit` 요청을 보낸 뒤
Ctrl+C와 같은 정상 종료 경로로 GUI와 command_input_node를 종료합니다.

`home`은 `/tool_command`로 발행되어 task node가 Home joint pose 이동과 gripper
open 흐름으로 처리합니다. 로봇이 이미 작업 중이면 즉시 끼어들지 않고 먼저
정지 후 복귀하도록 안내합니다.
...
LLM fallback을 쓰려면 Ollama 서버와 모델이 필요합니다. 설치가 끝난 환경에서는
아래처럼 모델과 서버를 확인합니다.
...
| --- | --- | --- | --- |
| `/tool_command` | command -> task | `std_msgs/String` JSON | bring/return/release/home 명령 |
| `/robot_task_control` | command -> task | `std_msgs/String` JSON | pause/resume/exit 작업 제어 |
...
```bash
ros2 topic pub --once /tool_command std_msgs/msg/String \
  "{data: '{\"tool_name\":\"unknown\",\"action\":\"home\",\"raw_text\":\"홈위치로 가\",\"match_method\":\"manual\",\"match_score\":1.0,\"confidence\":1.0,\"status\":\"accepted\"}'}"
```
...
일반 실행 중에는 GUI의 `복귀` 버튼 또는 “홈위치로 가” 음성/채팅 명령을 우선 사용하고,
아래 명령은 ROS topic 흐름이 동작하지 않을 때의 수동 fallback으로 사용합니다.
