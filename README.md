...
`macgyvbot.launch.py`는 로봇 메인 노드, hand grasp detection, STT, LLM command node를 함께 실행합니다. GUI 채팅창은 로그와 입력이 섞이지 않도록 별도 터미널에서 실행합니다.
...
GUI는 `/stt_text`, `/command_feedback`, `/tool_command`, `/target_label`을 확인하며 사용자 입력도 `/stt_text`로 발행합니다. GUI 실행에 PyQt5가 필요합니다.
...
```text
voice_command_gui_node 또는 stt_node
  -> /stt_text
  -> llm_command_node
  -> /tool_command
  -> /target_label
...
마이크 STT 없이 GUI 입력만 테스트하려면 Terminal 3에서 STT를 끄고 실행합니다.
...
마이크 STT 없이 GUI 입력만 확인할 때는 `macgyvbot.launch.py`에서 STT를 끄고 실행한 뒤, GUI를 별도 터미널에서 실행합니다.
