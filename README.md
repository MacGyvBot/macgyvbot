...
command pipeline은 typed `/stt_text` 입력을 받아 tool/task command를 발행하고,
operator UI는 ROS topic을 통해 command feedback, robot status, detector image를
표시합니다.
...
operator GUI의 Gripper Control 패널은 typed `/manual_gripper_width` topic으로
RG2 width 명령을 요청합니다. GUI는 slider release 또는 `적용` 버튼에서만 요청을
보내며, task coordinator는 작업 실행 중이거나 그리퍼 상태가 안전하지 않으면
하드웨어 명령을 거부합니다.

## Task Coordinator

`macgyvbot_task`는 command routing과 task execution을 분리합니다.
`macgyvbot_main_node`는 `/tool_command`를 받아 typed `/task_request`를
발행하는 경량 router입니다. `task_coordinator_node`는
