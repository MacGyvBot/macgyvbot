...
├── macgyvbot_perception/                # camera, detection, depth, hand grasp
├── macgyvbot_manipulation/              # MoveIt, gripper, force, safe workspace
├── macgyvbot_config/                    # shared Python runtime constants
├── macgyvbot_domain/                    # shared Python dataclasses
├── macgyvbot_resources/                 # calibration / weights install assets
├── macgyvbot_interfaces/                # shared msg/srv/action contracts
└── macgyvbot_ui/                        # future independent UI boundary
```

메인 task executable 이름은 `macgyvbot`으로 유지되며, 현재 entrypoint는
`macgyvbot_task.macgyvbot_main_node`입니다.

## Multi-Package Migration

`src/` 아래에는 워크스페이스형 다중 ROS 패키지로 전환된 실행 패키지가
있습니다. 저장소 루트는 ROS 패키지가 아니라 colcon workspace root입니다.

```text
src/
├── macgyvbot_interfaces/     # shared msg/srv/action
├── macgyvbot_config/         # shared Python runtime constants
├── macgyvbot_domain/         # shared Python dataclasses
├── macgyvbot_resources/      # calibration / weights install assets
├── macgyvbot_bringup/        # launch/config wiring
├── macgyvbot_command/        # command input, parser, STT/TTS
├── macgyvbot_perception/     # camera, detection, depth, hand grasp
├── macgyvbot_manipulation/   # MoveIt, gripper, force, safe workspace
├── macgyvbot_task/           # pick/return workflow orchestration
└── macgyvbot_ui/             # operator UI adapters
```

전환 원칙은 기존 파일을 즉시 삭제하거나 복제하지 않고, 한 책임 단위씩 새 패키지로 이동한 뒤 launch/build/test 계약을 확인하는 방식입니다.

현재 전환 상태:

- `macgyvbot_command`는 command input node, parser, STT/TTS helper, command GUI widget을 소유합니다.
- `macgyvbot_config`는 여러 패키지가 공유하는 Python runtime constant를 소유합니다.
- `macgyvbot_domain`은 package 간 공유되는 Python dataclass를 소유합니다.
- `macgyvbot_resources`는 calibration과 model asset 설치 경로를 소유합니다.
- `macgyvbot_perception`는 perception helper와 hand grasp detection node를 소유합니다.
- `macgyvbot_manipulation`은 MoveIt, gripper, force, pose, safe workspace helper를 소유합니다.
- `macgyvbot_task`는 main task node와 pick/return workflow를 소유합니다.
- `macgyvbot_bringup`의 `macgyvbot.launch.py`가 기본 실행 entrypoint입니다.
- 저장소 루트에는 ROS 패키지를 두지 않고, 모든 runtime 패키지는 `src/` 아래에 둡니다.
...
flowchart TD
    ...
    subgraph handgrasp["hand_grasp_detection_node"]
        ...
        handDet["perception/hand_tool_grasp/hand_detector.py\nMediaPipe hand landmarks"]
        toolDet["perception/hand_tool_grasp/tool_detector.py\nYOLO tool ROI detection"]
        calc["perception/hand_tool_grasp/calculations.py\ngeometry / depth / hand selection helpers"]
        mlGrasp["perception/hand_tool_grasp/ml_grasp_classifier.py\nML hand grasp classifier"]
        mask["perception/hand_tool_grasp/sam_tool_mask.py\noptional tool mask diagnostics"]
        viz["perception/hand_tool_grasp/visualization.py\noverlay rendering"]
        handDet --> hgNode
        toolDet --> hgNode
        calc --> hgNode
        calc --> handDet
    ...
    subgraph mainbot["macgyvbot_main_node"]
        ...
        graspSel["perception/grasp_point/grasp_point_selector.py\ncenter/VLM grasp-point routing"]
        vlm["perception/grasp_point/vlm_grasp_point_selector.py\nVLM grasp pixel inference"]
...
MacGyvBot 워크스페이스 패키지만 빌드:

```bash
cd ~/ros2_ws
colcon build --packages-up-to macgyvbot_bringup
```
...
```bash
ros2 run macgyvbot_command command_input_node --ros-args -p enable_microphone:=true
```
...
```bash
ros2 launch macgyvbot_bringup macgyvbot.launch.py yolo_model:=/path/to/yolov11_best.pt
```
...
ML 모델은 Git에 포함하지 않습니다. 기본 설치 경로는
`share/macgyvbot_resources/weights/hand_grasp_model.pkl`이며, 소스 트리에서는
`src/macgyvbot_resources/weights/hand_grasp_model.pkl`에 둘 수 있습니다. 다른
경로를 쓸 때는 launch 파라미터로 넘깁니다.
...
MobileSAM checkpoint는 다음 명령으로 준비합니다. 모델 파일은
`src/macgyvbot_resources/weights/`에 저장되지만 Git에는 포함하지 않습니다.
...
```bash
colcon test --packages-up-to macgyvbot_bringup
