...
있습니다. 코드 구조를 먼저 보고 싶다면 [EXPLAIN.md](./EXPLAIN.md)를 읽습니다.

## 현재 기본값

- 기본 실행 entrypoint는 `ros2 launch macgyvbot_bringup macgyvbot.launch.py`입니다.
- 기본 grasp point mode는 `vlm`입니다.
- VLM 추론 또는 depth refinement가 실패하면 bbox center grasp point로 fallback합니다.
...
├── macgyvbot_task/           # main task node, task queue, pick/return workflow
...
├── macgyvbot_interfaces/     # shared typed msg migration targets
...
MacGyvBot은 홈 디렉터리 아래에 단독 colcon workspace로 clone해서 사용하는 것을
기본 예시로 둡니다. 다른 경로에 clone해도 workspace root에서 같은 명령을 쓰면
됩니다.
...
기본 실행은 `grasp_point_mode:=vlm`, `sam_enabled:=true`,
`use_voice_command:=true`, `use_stt:=true`, `use_tts:=true`입니다.
...
command input node를 빼고 task/perception만 실행:

```bash
ros2 launch macgyvbot_bringup macgyvbot.launch.py use_voice_command:=false
```
...
기본 command parser가 명령 대상으로 확정하는 공구는 `screwdriver`, `pliers`,
`hammer`, `tape_measure`입니다. hand grasp detection의 YOLO tool class filter는
handoff perception을 위해 `drill`, `hammer`, `pliers`, `screwdriver`,
`tape_measure`, `wrench`까지 볼 수 있습니다.
...
| `llm_timeout_sec` | `25.0` | Ollama 요청 timeout |
| `tts_engine` | `auto` | `auto`, `edge`, `espeak-ng` |
| `tts_voice` | `ko-KR-SunHiNeural` | edge-tts voice |
...
| `/robot_task_control` | command/manual -> task | `std_msgs/String` JSON/plain | stop/pause/resume 작업 제어 |
...
| `/hand_grasp_detection/annotated_image` | perception -> debug/task | `sensor_msgs/Image` | hand grasp overlay |
...
| `/stt_result` | command -> legacy | `std_msgs/String` | 수업 예제 호환 STT topic |
...
ros2 topic pub --once /robot_task_control std_msgs/msg/String \
  "{data: '{\"action\":\"stop\",\"reason\":\"manual_test\"}'}"
```
...
pick/return 작업은 `TaskControlCoordinator`가 worker thread에서 step queue로
실행합니다. 실행 중 새 작업이 들어오면 `busy` 상태가 발행됩니다.

`/robot_task_control`은 아래 action을 받습니다.

- `stop`: queue를 비우고 가능한 MoveIt goal cancel을 요청합니다.
- `pause`: pause event를 set하고 가능한 motion cancel을 요청합니다.
- `resume`: pause event를 clear하고 재개 상태를 발행합니다.

현재 자연어 parser는 “멈춰/정지/중단/stop/pause” 같은 표현을 `stop`으로
해석합니다. `pause/resume`은 topic-level 제어 경로로 사용할 수 있습니다.
...
기본값은 5회입니다. pick 실패는 `reason=robot_grasp_failed`, return 실패는
...
있습니다. 기본 접촉 기준은 `RETURN_HOME_FORCE_THRESHOLD_N = 8.0`입니다.
