...
`exit` 명령 흐름으로 해석한 뒤 먼저 `pause`를 발행합니다. 로봇이 안전하게
멈췄거나 대기 중임을 상태로 확인한 경우에만 `home`을 발행하고, Home 복귀 완료
상태를 받은 뒤 GUI와 command node를 종료합니다.
...
`pause`가 들어오면 command GUI는 정지 요청을 로봇 노드로 전달한 뒤
“작업을 재개할까요, 아니면 종료할까요?”라고 묻고 `재개` / `종료` quick reply
버튼을 표시합니다. 현재 `resume`은 제어 인터페이스 연결 전까지 GUI 안내만
표시합니다. `exit`는 command node가 `pause`를 먼저 발행하고, 로봇 상태가
`cancelled`, `paused`, `done` 등 안전한 상태로 바뀐 뒤 `home`을 요청합니다.
로봇이 MoveIt 동작 중이라 `busy`를 반환하면 GUI를 닫지 않고 종료를 보류합니다.
...
| --- | --- | --- | --- |
| `/tool_command` | command -> task | `std_msgs/String` JSON | bring/return/release/pause/home 명령 |
