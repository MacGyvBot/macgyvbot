...
- `src/macgyvbot_task`
  ...
  - `/robot_task_status`를 발행하고 `/robot_task_control`로 stop/pause/resume
    요청을 받습니다.
  - stop/pause 시 현재 `FollowJointTrajectory` goal을 직접 cancel하고, queue
    상태는 `TaskControlCoordinator`가 관리합니다.
...
- `src/macgyvbot_manipulation`
  ...
  - MoveItPy는 planning에 사용하고, 실행은
    `/dsr_moveit_controller/follow_joint_trajectory` action goal로 직접 보냅니다.
  - MoveIt/trajectory action/그리퍼/힘 감지 같은 하드웨어 adapter 세부 구현은
    이 패키지에 둡니다.
...
macgyvbot_command.command_input_node
  ...
  -> stop/pause/resume 제어는 /robot_task_control 발행
...
macgyvbot_task.macgyvbot_main_node
  ...
  -> MoveItPy로 plan, FollowJointTrajectory action으로 실행
...
```text
command_input_node._handle_text()
  -> /tool_command publish
macgyvbot_main_node._tool_command_cb()
  -> ToolCommandController.handle_command()
  -> target_label 설정
PickFrameProcessor.handle_target_detection()
  -> start_pick_sequence()
TaskControlCoordinator.start_pick()
  -> PickSequenceRunner.build_steps()
  -> queue에 TaskStep 적재
TaskControlCoordinator._run()
  -> step을 하나씩 pop/execute
MoveItController.plan_and_execute()
  -> MoveItPy plan
  -> /dsr_moveit_controller/follow_joint_trajectory goal 실행
```

현재 pick step은 대략 아래 순서입니다.

```text
pick/open_gripper
pick/travel_z
pick/xy_move
pick/apply_vlm_yaw          # VLM yaw가 있을 때만 추가
pick/approach
pick/grasp_descent
pick/grasp_tool
pick/wait_tool_mask_lock
pick/lift
pick/move_to_handoff
pick/wait_human_grasp
pick/release_to_human
pick/home_after_handoff
pick/done
```
...
return flow도 pick flow와 같은 task queue 계약을 사용합니다. stop/pause가 걸리면
현재 motion goal을 cancel하고, pause로 실패한 step은 queue front에 다시 넣은 뒤
resume을 기다립니다.
...
- `stop`: main node가 현재 `FollowJointTrajectory` goal handle에 cancel을 요청하고,
  `TaskManagement`가 stop event를 set한 뒤 pending queue를 비웁니다.
- `pause`: main node가 현재 `FollowJointTrajectory` goal handle에 cancel을 요청하고,
  pause event를 set합니다. queue는 비우지 않습니다.
...
while running:
  stop_event가 있으면 종료
  pause_event가 이미 있으면 새 step 시작 없이 대기
  step = queue.popleft()
  ok = step.execute()
  ok이면 다음 step
  pause_event가 있고 step.retry_on_pause이면 queue 앞에 step 재삽입
  stop_event가 있으면 종료
  그 외 False는 실패 종료
```

즉 stop/pause/resume의 의도된 동작은 아래와 같습니다.

```text
stop
  -> current FollowJointTrajectory goal cancel
  -> pending queue clear
  -> worker 종료
...
pause
  -> current FollowJointTrajectory goal cancel
  -> pending queue 유지
  -> 실행 중이던 step이 pause로 False를 반환하면 queue front에 재삽입
  -> worker는 pause_event가 clear될 때까지 대기
...
현재 자연어 parser는 명확한 정지/일시정지/재개 표현을 `/robot_task_control`로
보냅니다. 수동 테스트도 같은 topic을 사용합니다.
