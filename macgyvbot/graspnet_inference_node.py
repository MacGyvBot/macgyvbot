...
class GraspNetInferenceNode(Node):
    def __init__(self):
        ...
        self.voxel_size = float(self.declare_parameter("voxel_size", 0.01).value)
    ...
    def _load_graspnet_model(self):
        ...
        baseline_path = os.path.abspath(os.path.expanduser(self.graspnet_baseline_path))
        if not os.path.isdir(baseline_path):
            self.get_logger().error(f"graspnet_baseline_path not found: {baseline_path}")
            return None

        checkpoint_path = os.path.abspath(os.path.expanduser(self.checkpoint_path))
        if not os.path.isfile(checkpoint_path):
            self.get_logger().error(f"checkpoint_path not found: {checkpoint_path}")
            return None

        for relative_path in reversed(("models", "dataset", "utils", "pointnet2", "knn")):
        ...
        try:
            ...
            self.get_logger().error(f"Failed to import GraspNet dependencies: {exc}")
            return None
        ...
        try:
            ...
            self.get_logger().error(f"Failed to load GraspNet checkpoint: {exc}")
            return None
    ...
    def _select_device(self, torch):
        if self.device_name.startswith("cuda"):
            ...
            self.get_logger().warn("CUDA was requested but is not available. Using CPU.")
    ...
    def predict_grasp_pose(
        ...
        if self.model is None:
            self.get_logger().warn(
                "GraspNet model is not available. No grasp pose will be published.",
        ...
        if color_msg is None or depth_msg is None or camera_info_msg is None:
            self.get_logger().warn(
                "Waiting for color, depth, and camera_info before GraspNet inference.",
                throttle_duration_sec=5.0,
            )
            return None
    ...
    def _prepare_graspnet_input(
        ...
        if len(cloud_masked) == 0:
            raise RuntimeError("no valid depth points in configured depth range")

        sample_count = max(self.num_point, 1)
        if len(cloud_masked) >= sample_count:
            idxs = np.random.choice(len(cloud_masked), sample_count, replace=False)
        else:
    ...
    def _select_best_grasp(self, grasp_group, target_label: str):
        ...
        grasp = grasp_group[0]
        label_text = f" for target_label={target_label}" if target_label else ""
        self.get_logger().info(
            f"Selected GraspNet grasp{label_text}: score={grasp.score:.3f}, "
            f"width={grasp.width:.3f}, translation={grasp.translation.tolist()}"
        )
        return grasp
