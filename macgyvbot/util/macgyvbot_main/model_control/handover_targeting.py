...
def move_to_candidate_with_offset(
    ...
    for attempt_index, (safe_x, safe_y, safe_z) in enumerate(attempts, start=1):
        last_pose = SearchStartPose(safe_x, safe_y, safe_z)
        logger.info(
            "사용자 손 위치 전달 플래닝 시도: "
            f"{attempt_index}/{len(attempts)}, "
            f"target=({safe_x:.3f},{safe_y:.3f},{safe_z:.3f})"
        )
