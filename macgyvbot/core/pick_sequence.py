...
from macgyvbot.safety import clamp_to_safe_workspace
...
from macgyvbot.motion.pose_utils import (
...
class PickSequenceRunner:
    ...
    def run(self, bx, by, bz, z_m, vlm_yaw_deg=None):
        ...
        using_graspnet_pose = False

        graspnet_selector = getattr(self.state, "graspnet_pose_selector", None)
        if graspnet_selector is not None:
            graspnet_selector.wait_for_pose_if_needed(
                self.state.grasp_point_mode,
                log,
            )
            target_xyz, ori, using_graspnet_pose = graspnet_selector.select(
                (bx, by, bz),
                ori,
                log,
            )
