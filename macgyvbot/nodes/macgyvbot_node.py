...
from macgyvbot.perception.graspnet_pose_selector import GraspNetPoseSelector
...
class MacGyvBotNode(Node):
    def __init__(self):
        ...
        self.graspnet_config = self._read_graspnet_config()
        ...
        self.graspnet_pose_selector = GraspNetPoseSelector(
            self.robot,
            self.gripper2cam,
            pose_topic=self.graspnet_config["pose_topic"],
            pose_timeout_sec=self.graspnet_config["pose_timeout_sec"],
            wait_timeout_sec=self.graspnet_config["wait_timeout_sec"],
            target_distance_tolerance_m=self.graspnet_config[
    ...
    def _read_graspnet_config(self):
        self.declare_parameter("graspnet_pose_topic", GRASPNET_POSE_TOPIC)
        ...
        return {
            "pose_topic": str(
                self.get_parameter("graspnet_pose_topic").value
            ),
            "pose_timeout_sec": float(
                self.get_parameter("graspnet_pose_timeout_sec").value
            ),
            "wait_timeout_sec": float(
                self.get_parameter("graspnet_wait_timeout_sec").value
            ),
    ...
    def _create_subscriptions(self):
        ...
        self.create_subscription(
            PoseStamped,
            self.graspnet_config["pose_topic"],
    ...
    def _log_startup(self):
        ...
        self.get_logger().info(
            f"GraspNet pose 토픽: {self.graspnet_config['pose_topic']}"
        )
        self.get_logger().info(
    ...
    def _graspnet_pose_cb(self, msg):
        self.graspnet_pose_selector.update_pose(msg, self.get_logger())
